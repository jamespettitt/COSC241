...
public class Tableau {
    ...
    public void addValue(Integer value) {
        ...
        currCell = smallest;
        Integer test = addToRow(smallest,value);
        if(test == null){
            return;
        }else {
            if(smallest.below==null){
                Cell newCellOne = new Cell(test);
                smallest.below = newCellOne;
                newCellOne.above = smallest;
                
            } else {
                Integer newTest = test;
                do {
                    if( currCell.below == null){
                        Cell newCellTwo = new Cell(newTest);
                        currCell.below = newCellTwo;
                        newCellTwo.above = currCell;
                        return;
                    }
                    
                } while ((newTest = addToRow(currCell, newTest)) != null);
            }
    ...
    protected Integer addToRow(Cell curr, int value) {
        for(Cell c = curr; c!= null; c = c.right){
            ...
            if(c.right == null){
                Cell newCellOne = new Cell(value);
                c.right = newCellOne;
                newCellOne.left = c;
                if(c.above != null && c.right != null){
                    c.above.right.below = newCellOne;
                    newCellOne.above = c.above.right;
                }
                return null;
            }
        }
